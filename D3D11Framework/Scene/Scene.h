...
public:
   auto CreateCamera(const std::string& camera_name) -> std::shared_ptr<class Actor>;
   auto CreateDirectionalLight() -> std::shared_ptr<class Actor>;
   auto CreatePointLight()->std::shared_ptr<class Actor>;
   auto CreateSpotLight()->std::shared_ptr<class Actor>;
